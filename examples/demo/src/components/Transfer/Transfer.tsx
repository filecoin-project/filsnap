...
import { type FilecoinSnapApi } from '@chainsafe/filsnap-types'
...
export const Transfer: React.FC<ITransferProps> = ({
  ...
  const onSubmit = useCallback(async () => {
    if (amount && recipient && api) {
      ...
      if (signedMessageResponse.error != undefined) {
        showAlert('error', 'Error on signing message')
      } else if (
        signedMessageResponse.error == undefined &&
        !signedMessageResponse.confirmed
      ) {
        showAlert('info', 'Signing message declined')
      } else {
        showAlert(
          'info',
          `Message signature: ${signedMessageResponse.signedMessage.signature.data}`
        )
        const txResult = await api.sendMessage(
          signedMessageResponse.signedMessage
        )
        ...
        <Grid container alignItems="center" justifyContent="space-between">
          <Grid item xs={12}>
            ...
            <TextField
              InputProps={{
                startAdornment: (
                  <InputAdornment position="start">FIL</InputAdornment>
                ),
              }}
            ...
            <TextField
              ...
              fullWidth
              id="maxfee"
              label="Max fee (0.1 FIL if not set)"
              variant="outlined"
              value={maxFee}
            ></TextField>
          </Grid>
